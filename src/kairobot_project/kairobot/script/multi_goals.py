...
# Pin Definitions
output_pin = 27  # BCM pin 18, BOARD pin 12
...
class MultiGoals:
    def __init__(self, goalListX, goalListY, goalListZ, loopTimes, map_frame):
        ...
        rospy.loginfo("Initial goal published! Goal ID sis: %d", self.goalId) 
        pesan = "Current Goal ID is: " + str(self.goalId)
    ...
    def statusCB(self, data):
        if self.loopTimes and (self.loop > self.loopTimes):
            rospy.loginfo("Loop: %d Times Finished", self.loopTimes)
            pesan = "Loop: " + str(self.loopTimes) + " Times Finished"
            fd = open("/home/kai/catkin_ws/src/kairobot_project/kairobot/path/log.txt", "w+")
            fd.write(str(pesan))
            fd.close()
	    ...
	    time.sleep(2)   
            ...
            sys.exit()
            os._exit()
            rospy.loginfo("not quit")
        if data.status.status >= 1 and (not self.wayPointFinished): # reached
